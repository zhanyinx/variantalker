...
What is asserted where, and why it is asserted *there*. Written as the closing ticket of issue 28 — the
...
| `test_row_recovery.py` | unit | no | **which** variant the dialog opens, by either route, and what it does when it cannot be sure: `__pandas_index` read as the position `st_aggrid` builds it as rather than as a label — asserted against the installed component, so an upgrade that changes it fails here instead of quietly missing — on frames labelled the way a report's passed/failed split leaves them, which is where reading it as a label opened a different variant; every position exercised including 0, the one falsy ordinal, on a frame whose repeated `(gene, chromosome, position)` triple stops the column match covering for a truthiness test; both routes over the same frames, since the button receives that value as the row's *name* and the double-click as a field; and the three-column fallback held to answering only an unambiguous match — a partial row with none of the three must not be answered with row 0 of the report, nor an ambiguous one with the first of its matches, that being the ordinary case in 37 of this machine's 104 real MAFs |
...
| `test_launcher_dependencies.py` | guard | no | installing and launching land in the same interpreter: one variable through both scripts and the Makefile, pip/streamlit/pytest reached as `-m` modules rather than as PATH entries of their own, a launcher that checks before it boots, and a checker that really reads `requirements.txt` — plus what that file may declare: every line pinned to one exact version with the reason for the number beside it, the development extras still only comments, and every sweep in the class proving its own parse against a named anchor before it asserts, since a sweep that read nothing would run zero iterations and pass. Also the pin pattern itself, tabled in both directions — a floor or a marker must not read as pinned, and a legal exact pin with extras or spaces must not read as unpinned — and the Python floor the pins impose (`numpy==2.3.1` needs 3.11), held against the prerequisite the README promises. Since issue 258 that interpreter is a virtual environment in the checkout, so the resolution is asserted by *running* it against stub environments — the venv preferred, the Windows `Scripts/` layout found, an interpreter that cannot be run refused, `MAFIGATE_PYTHON` short-circuiting — plus the rule that nothing but `mafigate_python.sh` may spell the path, which the Makefile is held to as well |
| `test_mac_launcher.py` | guard | no | the mac launcher's venv surviving the app having moved (issues 411, 412, 414, 417) — the field failure where `mafigate-v1.0.0`'s second launch died on a dangling symlinked venv, driven through `launch.sh`'s own `ensure_venv()` behind its `MAFIGATE_LAUNCH_LIB` seam against a movable copy of this run's interpreter, dialogs stubbed and `HOME` sandboxed: a relaunch from a new path yields a working venv with the stale one's sentinel gone; a broken first-release venv heals with no manual cleanup; a still-working venv carrying the old version-arch stamp is rebuilt whole rather than kept bundle-pinned, `.deps_installed` going with it; and `pyvenv.cfg`'s `home` lands under `~/.mafigate` and never inside the app bundle — asserted where a copied base anchors to itself and skipped loudly, naming its siblings, where a framework copy re-anchors to the original install. Plus the structural rule the fix generalises, read off the script: every branch that sets `NEED_NEW_VENV=true` clears the stale venv first, because `python3 -m venv` over a stale directory skips existing symlinks, even dangling ones; and the seam itself, `ensure_venv` defined behind the lib guard so the tests and the launcher load one implementation. Skips whole on Windows, whose launcher is immune by construction (issue 413) and guarded in `test_windows_installer.py` |
| `test_delivery_channels_copy.py` | guard | no | what the README may promise about each delivery channel, tied to whether the tree can keep the promise yet: no link to a release artifact while `build/RELEASES.md` records no published release, and — the other direction, added when issue 264 re-keyed this switch off the mere *presence* of a release workflow — no copy still calling the installers unavailable once one is recorded, since the workflow's own arrival used to make both tests skip rather than flip. Both halves skip in one of the two states, so the flip is additionally **driven** over seeded copy — each rule evaluated against the state it must accept and the state it must refuse — because a dormant assertion is one nobody notices has stopped working, and this ticket's burden was that the guard still refuses both wrong states. Plus the ledger's parse, which must state one of its two states out loud, may not name a version ahead of `APP_VERSION`, and does not read a worked example inside an HTML comment as an event; the data-posture claim carried in one canonical table and qualified for exactly as long as no config turns Streamlit's usage reporting off; and a contributing note stating the one-way inbound flow and its manual credit |
| `test_release_preflight.py` | guard | no | the check that refuses to publish a draft the tree has moved on from, driven over states no live repository could be put into — reproducing them would mean drafting a release and then moving a branch under it, so `verdict` is a pure function of the six facts its caller had to go and find out. The case that matters is the one that actually happened: `mafigate-v1.0.0` drafted from what was then the tip of public `main`, twenty-two commits behind by the time anyone looked, four of them changing code that ships and one of those the fix for a dialog that opened the **wrong variant** — while the draft named its own commit accurately, hashed correctly, and read exactly as a fresh one reads. The load-bearing assertion is `test_it_refuses_even_when_nothing_that_ships_changed`: whether the intervening commits touched an installer's copy list decides how the refusal is *worded* and nothing else, because "the artifacts would probably be identical" is not what a version number claims — passing a docs-only drift is the one edit here that would silently restore the hole. Plus that the pass is reachable at all (or the refusals prove nothing), that an already-published release is *not* refused since a refusal with nothing left to protect teaches people to ignore it, and that both not-knowing paths — no release for the tag, and a repository `gh` cannot read — exit non-zero, since "I could not check" must never read like "I checked". **Rewritten by issue 405, which found the guard passing the incident it was written for.** The tag and the tip of the published branch are the *same commit* whenever nobody has exported since the tag was cut — the ordinary state, the export being manual and once per release — so comparing those two answered a question that is never the one at risk. It said *safe to publish* on the 328 incident, and again four days later with the private tree 74 commits ahead of what was published. The missing comparison is one level further out: public `main`'s own tip commit names the private commit it was exported from (the export writes that line; issue 227 made it the contract), and it must equal the private tree's HEAD. What let the hole hide was the **fixture, not an assertion**: the old case paired the real tag commit with a fabricated branch commit, describing a world in which the published branch had moved on, which it had not — so a guard that could not catch this looked like it had. Both real states are now encoded from the public repository's actual history, with `behind` 0 and no shipped changes, which is what makes them the hard cases; the one genuinely hypothetical fixture says so in its name. A public clone cannot answer the export question at all and returns *unknown* rather than *fine*, told apart by the export script, which the export strips — a discriminator with its own test, added after a mutation removing it went unnoticed |
| `test_release_page.py` | guard | no | what the release page must say, asserted on the page a reader gets rather than on the file on disk — the two placeholders CI fills mean the source is a document with a hole where the downloads go, so the assembly is run through `test_release_workflow`'s own step, imported rather than re-implemented for the reason issue 247 recorded: a check that copies a script's loop goes green on input the real one refuses. Four decisions of the delivery matrix are held to the page — the MAF prerequisite stated **above** the downloads (below them it is a note about why the download was wasted) and naming all three extensions the chooser accepts; the three failure modes, including the two that do not present as failures, an install that reports success and then will not start and a process the operating system stops with no message at all; issue 225's memory rule with both of its terms, so it can be applied to the file the reader has, plus what the largest MAFs cost in practice; and no update mechanism promised, with the absence said out loud, since it would be the app's first outbound call and would falsify the posture claim two paragraphs above it. The pinned data-posture sentence and the tree fact that earns it are both **imported** from `test_delivery_channels_copy`, so this page cannot keep promising something after that switch has flipped the other way. Deliberately not here: the first-open wording, which `test_unsigned_artifact_copy` sweeps to one home — the page carries a placeholder and would fail that sibling the moment it grew a paraphrase — and version literals, already swept by prefix out of `streamlit_app/build/`. Plus the assembly's own refusals, driven over seeded pages: a placeholder lost, duplicated or renamed, the failure that would otherwise ship a release page with no files named on it and nothing about it looking wrong |
...
| `test_public_export.py` | guard | no | the tree the public export publishes, held against what the deny-list and the scan gate claim about it — real exports into temporary repositories, asserting the notes and the private repository's name are absent, `.github/` and `bin/` present, the superseded public-only paths deleted, one commit naming the private SHA with no private history crossing; plus a seeded violation per scan rule to watch the gate refuse, and a case per rule that must *not* trip it. Also the one claim here about the **real** tree rather than a seeded one: a sweep of `git ls-files` minus the deny-list through the gate's own `scan_file`, so a case identifier committed on an ordinary day fails on that day instead of at release — with its own two vacuity checks, since a claim of zero is satisfied by a sweep that reads nothing, and a planted hit of *every* rule, since a matcher that had stopped seeing one rule would also report zero |
| `test_makefile_targets_are_runnable.py` | guard | no | that every script a recipe hands `$(PYTHON)` is present, and that no recipe names a path `.publicignore` strips — one claim read from both trees, because neither can see the defect alone. The presence half was false in **every public clone** for as long as `app-load-check` ran from `../docs/wayfinder/` (issue 345); the deny-list half is what refuses that here, where it is still cheap to fix. Anchored on two named scripts and a minimum count, so a Makefile whose shape changed fails rather than reading nothing, and the deny-list half **skips** where `.publicignore` is absent — which is exactly the exported tree — rather than passing vacuously |
...
The **needs `bin/`** column is not a description, it is a constraint, and the seven `yes` rows are
the complete list — `test_suite_organisation.py` names them and refuses an eighth without a
deliberate edit. It keys on the module's *name*, not on how it is filed, so nothing can exempt
itself by choosing a label. `make test-cov` then runs the whole suite in a tree with no `bin/` at all, so a
`net` or `unit` module that quietly acquired a pipeline dependency would either fail there or stop
asserting anything — which is issue 24's failure exactly: a module-level `skipif` removed the entire
...
**One row names a module a published clone does not have.** This table travels and the
one-way public export does not carry `test_public_export.py` — the export's own proof,
which needs the export script and its deny-list, both stripped with it. So in a published
tree that row points at nothing, and until issue 348 it made that tree's `make test` red.
The row stays: it is the honest account of what tests this suite, and deleting it would
make the table understate the app's coverage everywhere in order to be literally true in
one place. What changed is that the check now knows *which* absences are deliberate —
`MODULES_STRIPPED_FROM_THE_PUBLIC_TREE`, named there rather than inferred — and any other
missing module is still a failure. Where `.publicignore` is present the exemption has to
prove itself: the deny-list must really name the module, and the module must really be on
disk, so it cannot become a way to retire a test by leaving its row behind.
...
Publication goes through the public export, whose scan gate refuses to export any
