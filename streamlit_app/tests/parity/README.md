...
The measurement instrument the map's destination is defined in terms of (issue 17). It runs
...
| file | what it is |
|---|---|
| `contract.py` | the issue 11 parameter contract, its translation to each side, and the case matrix |
...
| `test_absent_columns.py` | the **one deliberate deviation**: a MAF missing a filter-input column, which the pipeline refuses and the app fills (issue 39) |
| `mutation_coverage.py` | the coverage instrument: re-injects each divergence into the app and records which cases notice (issue 242) |
...
instances of 'str' and 'int'` (issue 10). The harness therefore had to do its own loading, and could not
be read as validating the app's. Issue 32 deleted that loader — `utils.read_maf` now delegates to the
...
`Tumor_Sample_Barcode`, which is `__UNKNOWN__` in every germline reference row; issue 18's generator
...
Contract values are `nextflow.config`'s `params` block, with issue 11's single departure
...
- **`literal`** — the same list value to both, so the app keeps exactly what the pipeline
  drops. Documents divergence 1 at full size. Only the two `*_defaults_literal` cases use it.
- **`complement`** — the app gets its vocabulary minus the list, the closest expressible
  ...
  divergence 1's `vc_outside_app_vocabulary` counts rows the app's hardcoded vocabulary cannot
  express at all.
...
- **parity** (`test_row_parity`, `test_column_parity`) — the destination. Every case is
  ...
  **The ratchet is gone, and its removal is the last step of the work it governed.** While
  ...
  silently claimed. Issue 35 took `test_column_parity` off it first: column parity became
  *structural* (both sides run the same vendored `compute_keep`), so a case could not drift
  ...
  regression be absorbed by re-recording the baseline. Issue 41 made the same call for the rows,
...
refused by the pipeline (`KeyError`) and *filled and reported* by the app, by issue 39's deliberate
...
Plus: neutrality of the app-only frequency extra (issues 16 and 37) asserted **algebraically** with the `if
...
which made it a test of the copy — and the claim it carries got stronger with issue 37: the mask
...
Defaults alone are a weak test, and one structural fact from issue 18 shapes the matrix: at
...
all-empty state issue 13 made reachable, which is pipeline-expressible (`--filter_cancervar "" …`) and
...
`somatic_defaults` rows are matched by both divergence 4 `clnsig_split_vs_exact` and
divergence 7 `somatic_patho_no_clinvar_clause`, and fixing **only** divergence 7 brought the case to full
parity. Definitive attribution is "apply the fix and re-measure", which is exactly what
the ratchet supports.

**An empty `unattributed` bucket meant coverage, not completeness — until issue 27.** The sentence above
was true of the fixtures from the first baseline and it was misleading for the whole of that time.
Run over the 50-sample reference, issue 25 found **3,687** diverging rows no probe explained — the single
largest bucket — and every one of them was divergence **6**, the ESCAT clause the app ORs into the
*germline* guideline block. There was no divergence 6 probe, so the harness could not name it; and there was
...
InterVar, ClinVar *and* RENOVO — a textbook divergence 6 witness — selected by a cell named `escat_populated`.
...
the divergence 6 cell and pins the row's whole path to the verdict: rejected by all three pipeline guideline
sources, rescued by neither side, clearing depth on **both** columns, VAF and classification, and
inert under every other probe. The reference holds **536** such rows; issue 25's ablation figure of 540
...
Verified as a ratchet rather than assumed — against the baseline before issue 27 the divergence 6 probe
is named by no case at all, so the test failed; against the one after it, it was named by 9.
...
they are two cells of issue 20's decomposition:
...
Since issue 33 the claim is structural rather than measured, which is stronger: the app calls
...
even at zero. The reference fixtures cannot witness this — issue 18 measured zero blanks in
...
- **Fixtures, not the reference.** Every number in the tables above is from the ten
  ...
  parity-reference` (below). Issue 16's **411 vs 408** somatic-baseline gap **is reconciled**: it was
...
- **The fixture rates are not rates.** The subsample is purposive, so a divergence's
  ...
  Measured: divergence 6 is **51%** of the reference's attributed diverging rows and was **0%** of
  the fixtures'; divergence 4 is the fixtures' largest at a combined 33.6% and **3.9%** on the
  reference. The fixtures are still the only place divergence 3's NaN cells, divergence 7's dead ESCAT
...
- ~~**Column parity is measured against `config/columns.py`**~~ **Closed by
  issue 35.** It used to be measured against the app's stated `KEEP` analogue, which never filtered any
...
- ~~**The app does not fail where the pipeline fails.**~~ **Closed by issue 33, improved by
  issue 38.** On `somatic_dot_numeric.maf` the app used to return a verdict (0 PASS of 5) where
...
- **`compute_keep` is not vendored yet** (flagged by
  issue 19), so the harness compares against the pipeline's *emitted* header rather than against a
  vendored function.
...
everywhere by skipping — the exact failure issue 24 found in `test_parity.py`, whose module-level
...
1. **No diverging row is unattributed.** Precisely the property that was false by 3,687
   ...
   reason: with the divergence 6 probe removed, a two-sample sweep exits 1 on 19 unexplained rows;
   with it present, 0.
2. **The four-cell decomposition reproduces `bin/filter_variants.py` exactly** — 0
   ...
   no longer reconstruct the pipeline's verdict and every diagnostic built on them — issue 20's cells,
   issue 21's `MAFigate_reason` — is describing a decision the pipeline did not make.
...
Results are written with `--out`; the copy committed to the private notes for issue 27 is
scrubbed on issue 18's precedent (samples aliased, drive path dropped, germline coordinates
