...
def read_datasets():
    ...
    if os.path.isfile(paras["evidence_file"]):
        try:
            ...
            print(
                "Error: can't read the user specified evidence file %s"
                % paras["evidence_file"]
            )
        else:
            fh.close()
    ...
    try:
        ...
        print(
            "Error: can't read the cancervar_markers file %s"
            % paras["cancervar_markers"]
        )
        print("Error: Please download it from the source website")
        sys.exit()
        return
        ...
        for ii in range(0, len(cancervar_d)):
            ...
            mut_types = mut_type.split(";")
            # BIA CNA EXPR FUS MUT
            ...
            for jj in range(0, list_mut_size):
                ...
                if mut_types[jj] == "FUS":
                    ...
                    key = mutt
                    default_s = ""
                    cancervar_markers_dict[key] = (
                        str(ii) + "," + cancervar_markers_dict.get(key, default_s)
                    )
                    ...
                    # format as " exon(s) 10, 20, 21 any"
                        ...
                        if not re.findall(
                            ...
                            cancervar_markers_dict[key] = (
                                str(ii)
                                + ","
                                + cancervar_markers_dict.get(key, default_s)
                            )
    ...
    if os.path.isfile(paras["exclude_snps"]):
        try:
            ...
            print(
                "Error: can't read the user specified SNP list file %s"
                % paras["exclude_snps"]
            )
        else:
            fh.close()
    ...
    try:
        ...
        print(
            "Error: can't read the knownGeneCanonical  file %s"
            % paras["knowngenecanonical"]
        )
        print("Error: Please download it from the source website")
        sys.exit()
    else:
    ...
    try:
        ...
        print(
            "Error: can't read the cancer_pathway genes file %s"
            % paras["cancer_pathway"]
        )
        print("Error: Please download it from the source website")
        sys.exit()
        return
    ...
    try:
        ...
        print(
            "Error: can't read the cancers diseases genes file %s"
            % paras["cancer_pathway"]
        )
        print("Error: Please download it from the source website")
        sys.exit()
...
def check_downdb():
    ...
    for dbs in ds.split():
        ...
        dataset_file = (
            paras["database_locat"] + "/" + paras["buildver"] + "_" + file_name + ".txt"
        )
        if dbs != "rmsk":
            cmd = (
                "perl "
...
def check_input():
    ...
    if inputft.lower() == "vcf":
        if os.path.isfile(paras["convert2annovar"]):
            ...
            print(
                "Error: The Annovar file [ %s ] is not here,please download ANNOVAR firstly: http://www.openbioinformatics.org/annovar"
                % paras["convert2annovar"]
            )
            sys.exit()
    if inputft.lower() == "vcf_m":
        if os.path.isfile(paras["convert2annovar"]):
            ...
            print(
                "Warning: Begin to convert your vcf file with multiple samples of %s to AVinput of Annovar with All.raw.highqc.vcf.<samplename>.avinput..."
                % paras["inputfile"]
            )
            print(
                "Warning: Please attention that the sample names in VCF file should  contain letters/numners only, otherwise the converting may be failure!"
            )
            print("%s" % cmd)
            os.system(cmd)
        else:
...
def check_genes(anvfile):
    ...
    try:
        ...
        for line in strs.split("\n"):
            cls = line.split("\t")
            if len(cls) > 1:
                if sum == 0 and re.findall(
                    "true", paras["otherinfo"], flags=re.IGNORECASE
                ):
                ...
                for gg in gene_name.split(","):
                    ...
                    if sum > 1:
                        line_out = re.sub("^[Cc][Hh][Rr]", "", line_out)
                    fw.write("%s\t\n" % line_out)

    except IOError:
        print(
            "Error: can't read/write the annovar output file %s %s"
            % (anvfile, newoutfile)
        )
        sys.exit()
        return
...
def classfy(CBP, Allels_flgs, cls):
    ...
    if os.path.isfile(paras["evidence_file"]):
        ...
        try:
            ...
            for evd in evds.split(";"):
                evd_t = evd.split("=")
                if len(evd_t) > 1 and re.findall(
                    "grade", evd_t[0], flags=re.IGNORECASE
                ):
                    ...
                    if int(evd_t[1]) <= 3:
                        ...
                        if evd_t[0].find("CBP") != -1:
                            ...
                            tt = evd_t[0]
                            tt3 = int(tt[t + 3 : t + 4])
                            if t < len(evd_t[0]) - 2 and tt3 <= 12:
                                CBP[tt3 - 1] = int(evd_t[1])
        except KeyError:
            pass
        else:
            pass
    ...
    if CBP_sum <= 2:
        BPS_out = 2  # benign
    if CBP_sum >= 3 and CBP_sum < 8:
        BPS_out = 3  # VUS
    if CBP_sum >= 8 and CBP_sum <= 10:
        BPS_out = 1  # potential path
    if CBP_sum >= 11:
        BPS_out = 0  # strong path
...
def check_Thera(line, Funcanno_flgs, Allels_flgs, lof_genes_dict):
    ...
    for cls0 in line_tmp2_sp:
        ...
        if len(cls0) > 1 and len(line_tmp2_sp) > 0 and len(cls0_1) >= 4:
            ...
            default_s = ""
            add_list = (
                cancervar_markers_dict.get(marker_key, default_s)
                + cancervar_markers_dict.get(marker_key0, default_s)
                + cancervar_markers_dict.get(marker_key00, default_s)
                + cancervar_markers_dict.get(marker_key1, default_s)
                + cancervar_markers_dict.get(marker_key11, default_s)
                + cancervar_markers_dict.get(marker_key2, default_s)
                + cancervar_markers_dict.get(marker_key22, default_s)
            )
            cgi_list = add_list
            # out_list="";
            level_AB = "0"
            ...
            for i in cgi_list.split(","):
                ...
                if len(i) > 0:
                    ...
                    if cancervar_d[pos][9] == "Therapeutic":
                        ...
                        if t_level == "A" and level < 2:
                            level = 2
                        if t_level == "B" and level < 2:
                            level = 2
                        if t_level == "C" and level < 1:
                            level = 1
                        if t_level == "D" and level < 1:
                            level = 1
                        # print cancer_type,cancervar_d[pos][8]
                        if not re.findall(
                            cancer_type, cancervar_d[pos][8], flags=re.IGNORECASE
                        ):
                            if (
                                t_level == "A" or t_level == "B"
                            ):  # did not find the specific type, decrease level
                                level = 1
...
def check_Mut(line, Funcanno_flgs, Allels_flgs, lof_genes_dict):
    ...
    try:
        if (
            float(cls[Funcanno_flgs["dbscSNV_RF_SCORE"]]) > dbscSNV_cutoff
            or float(cls[Funcanno_flgs["dbscSNV_ADA_SCORE"]]) > dbscSNV_cutoff
        ):
            VS_t3 = 1
    except ValueError:
        pass
...
def check_PopD(line, Freqs_flgs, Funcanno_flgs, Allels_flgs, lof_genes_dict):
    ...
    for key in Freqs_3pops.keys():
        try:
            if cls[Freqs_flgs[key]] != "." and float(cls[Freqs_flgs[key]]) > 0.01:
                PopD = 0  #  MAF>1%
            if cls[Freqs_flgs[key]] != "." and float(cls[Freqs_flgs[key]]) < MAF_cutoff:
                PopD = 1  #  extremely low MAF
...
def check_PreP(line, Funcanno_flgs, Allels_flgs, lof_genes_dict):
    ...
    if (
        cls[Funcanno_flgs["Polyphen2_HDIV_pred"]] == "P"
        or cls[Funcanno_flgs["Polyphen2_HDIV_pred"]] == "D"
    ):
        dam = dam + 1
    ...
    if (
        cls[Funcanno_flgs["MutationAssessor_pred"]] == "H"
        or cls[Funcanno_flgs["MutationAssessor_pred"]] == "M"
    ):
        dam = dam + 1
    if (
        cls[Funcanno_flgs["MutationAssessor_pred"]] == "L"
        or cls[Funcanno_flgs["MutationAssessor_pred"]] == "N"
    ):
        ben = ben + 1
...
def assign(BP, line, Freqs_flgs, Funcanno_flgs, Allels_flgs):
    ...
    Population_data = check_PopD(
        line, Freqs_flgs, Funcanno_flgs, Allels_flgs, lof_genes_dict
    )
    ...
    if os.path.isfile(paras["evidence_file"]):
        ...
        try:
            ...
            for evd in evds.split(";"):
                evd_t = evd.split("=")
                if len(evd_t) > 1 and (
                    not re.findall("grade", evd_t[0], flags=re.IGNORECASE)
                ):
                    if int(evd_t[1]) <= 1:
                        # print ("%s %s %s " %(keys,evd_t[1],evd_t[0]))
                        if evd_t[0].find("CBP") != -1:
...
def my_inter_var_can(annovar_outfile):
    ...
    Funcanno_flgs = {
        ...
        "AAChange.refGene": 0,
        "Gene": 0,
        "Gene damage prediction (all disease-causing genes)": 0,
        "CLNDBN": 0,
        "CLNACC": 0,
        "CLNDSDB": 0,
        "dbscSNV_ADA_SCORE": 0,
        "dbscSNV_RF_SCORE": 0,
        "GERP++_RS": 0,
        "LoFtool_percentile": 0,
        "Interpro_domain": 0,
        "rmsk": 0,
        "SIFT_score": 0,
        "phastCons20way_mammalian": 0,
        "Gene.ensGene": 0,
        "CLNSIG": 0,
        "CADD_raw": 0,
        "CADD_phred": 0,
    ...
    try:
        ...
        if re.findall("true", paras["otherinfo"], flags=re.IGNORECASE):
            fw.write(
                "#%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t CancerVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
                % (
                    "Chr",
                    "Start",
                    "End",
            ...
            fw.write(
                "#%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t CancerVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
                % (
                    "Chr",
                    "Start",
                    "End",
        ...
        for line in strs.split("\n"):
            ...
            if line_sum == 0:
                ...
                cancervar_bp, Therap_list, Diag_list, Prog_list = assign(
                    BP, line, Freqs_flgs, Funcanno_flgs, Allels_flgs
                )
                # print Therap_list
                Freq_ExAC_POPs = (
                    "AFR:"
                    + cls[Freqs_flgs["ExAC_AFR"]]
                    + ",AMR:"
                    + cls[Freqs_flgs["ExAC_AMR"]]
                    + ",EAS:"
                    + cls[Freqs_flgs["ExAC_EAS"]]
                    + ",FIN:"
                    + cls[Freqs_flgs["ExAC_FIN"]]
                    + ",NFE:"
                    + cls[Freqs_flgs["ExAC_NFE"]]
                    + ",OTH:"
                    + cls[Freqs_flgs["ExAC_OTH"]]
                    + ",SAS:"
                    + cls[Freqs_flgs["ExAC_SAS"]]
                )
                Freq_gnomAD_genome_POPs = (
                    "AFR:"
                    + cls[Freqs_flgs["gnomAD_genome_AFR"]]
                    + ",AMR:"
                    + cls[Freqs_flgs["gnomAD_genome_AMR"]]
                    + ",EAS:"
                    + cls[Freqs_flgs["gnomAD_genome_EAS"]]
                    + ",FIN:"
                    + cls[Freqs_flgs["gnomAD_genome_FIN"]]
                    + ",NFE:"
                    + cls[Freqs_flgs["gnomAD_genome_NFE"]]
                    + ",OTH:"
                    + cls[Freqs_flgs["gnomAD_genome_OTH"]]
                    + ",ASJ:"
                    + cls[Freqs_flgs["gnomAD_genome_ASJ"]]
                )
                OMIM = "."
                ...
                if re.findall("true", paras["otherinfo"], flags=re.IGNORECASE):
                    fw.write(
                        ...
                        % (
                            ...
                            cls[Freqs_flgs["esp6500siv2_all"]],
                            cls[Freqs_flgs["1000g2015aug_all"]],
                            cls[Freqs_flgs["gnomAD_genome_ALL"]],
                            cls[Funcanno_flgs["CADD_raw"]],
                            cls[Funcanno_flgs["CADD_phred"]],
                            cls[Funcanno_flgs["SIFT_score"]],
                            cls[Funcanno_flgs["GERP++_RS"]],
                            cls[Funcanno_flgs["phastCons20way_mammalian"]],
                            cls[Funcanno_flgs["dbscSNV_ADA_SCORE"]],
                            cls[Funcanno_flgs["dbscSNV_RF_SCORE"]],
                            cls[Funcanno_flgs["Interpro_domain"]],
                            cls[Funcanno_flgs["AAChange.knownGene"]],
                            cls[Funcanno_flgs["MetaSVM_score"]],
                            Freq_gnomAD_genome_POPs,
                            OMIM,
                            Pheno_MIM,
                            orpha,
                            ...
                            Diag_list,
                            Prog_list,
                            cls[Funcanno_flgs["Polyphen2_HDIV_score"]],
        ...
        print(
            "Error: can't readi/write the annovar output files %s"
            % (newoutfile, newoutfile2)
        )
        sys.exit()
        return
    else:
...
def main():
    ...
    parser.add_option(
        "-v", action="version", help=optparse.SUPPRESS_HELP, dest="version"
    )
    ...
    parser.add_option(
        ...
        dest="config",
        action="store",
        help="The config file of all options. it is for your own configure file.You can edit all the options in the configure and if you use this options,you can ignore all the other options bellow",
        metavar="config.ini",
    )
    ...
    parser.add_option(
        ...
        dest="buildver",
        action="store",
        help="The genomic build version, it can be hg19 and  hg38",
        metavar="hg19",
    )
    ...
    parser.add_option(
        ...
        dest="input_type",
        action="store",
        help="The input file type, it can be  AVinput(Annovar's format),VCF(VCF with single sample),VCF_m(VCF with multiple samples)",
        metavar="AVinput",
    )
    ...
    parser.add_option(
        ...
        dest="output",
        action="store",
        help="The prefix of output file which contains the results, the file of results will be as [$$prefix].cancervar ",
        metavar="example/myanno",
    )
    ...
    group.add_option(
        "-t",
        "--database_cancervar",
        dest="database_cancervar",
        action="store",
        help="The  database location/dir for the CancerVar dataset files",
        metavar="cancervardb",
    )
    group.add_option(
        "-s",
        ...
        dest="evidence_file",
        action="store",
        help="User specified Evidence file for each variant",
        metavar="your_evidence_file",
    )
    ...
    group.add_option(
        "--skip_annovar",
        action="store_true",
        help="Skip the Annovar annotation, this can be true only after you  already got the annovar's annotation results",
        dest="skip_annovar",
    )
    ...
    if os.path.isfile(config_file):
        ...
        print(
            "Error: The default configure file of [ config.ini ] is not exit! Please redownload the CancerVar."
        )
        sys.exit()
    ...
    if options.evidence_file != None:
        ...
        print(
            "Warning: You provided your own evidence file [ %s ] for the CancerVar."
    ...
    if options.database_cancervar != None:
        ...
        paras["knowngenecanonical"] = (
            paras["database_cancervar"] + "/knownGeneCanonical.txt"
        )
    ...
    if not os.path.isfile(paras["evidence_file"]) and paras["evidence_file"] != "None":
        print(
            "Warning: Your specified evidence file [ %s ] is not here,please check the path of your evidence file."
            % paras["evidence_file"]
        )
        print("         Your analysis will begin without your specified evidence.")
    else:
        print(
            "Warning: Your specified evidence file [ %s ], the analysis will take your additional evidence."
            % paras["evidence_file"]
        )
    ...
    try:
        print(
            "Warning: Your specified the cancer type: %s in command, the cancer type in otherinfo column will be replaced for all your variants!!!"
            % paras["cancer_type"]
        )
    except KeyError:
        pass
    ...
    if options.skip_annovar != True:
        ...
        print(
            "Warning: The CancerVar will interpret the variants based on your old annotation information!"
        )
    ...
    for annovar_outfile in glob.iglob(
        paras["outfile"] + "*." + paras["buildver"] + "_multianno.txt"
    ):
        ...
        if os.path.isfile(outfile):
            ...
            print(
                "Notice: About %d variants has been processed by CancerVar" % (sum2 - 1)
            )
            if inputft.lower() != "vcf_m":
                print(
                    "Notice: The CancerVar is finished, the output file is [ %s.cancervar ]"
                    % annovar_outfile
                )
        else:
            some_file_fail = some_file_fail + 1
            print(
                "Warning: The CancerVar seems not run correctly, please check your inputs and options in configure file"
            )

    if inputft.lower() == "vcf_m":
        print(
            "Notice: The CancerVar for VCF with multiple samples is finished, the output file is as [ %s.<samplename>.cancervar ]"
            % annovar_outfile
        )
        sum_sample = 1
        for f in glob.iglob(
            paras["outfile"] + "*." + paras["buildver"] + "_multianno.txt.cancervar"
        ):
            print(
                "Notice: The CancerVar for VCF with multiple samples is finished, The %d sample output file is [ %s]"
                % (sum_sample, f)
            )
            sum_sample = sum_sample + 1
        if some_file_fail >= 1:
            print(
                "Warning: The CancerVar seems not run correctly for your %d samples in the VCF, please check your inputs and options in configure file"
                % some_file_fail
            )
    if out_annf == 0:
        print(
        ...
        print(
            "ERROR: The name of annotation result file should be like %s*.%s_multianno.txt"
