...
def read_datasets():
    ...
    try:
        ...
        print(
            "Error: can't read the OMIM recessive disorder file %s"
            % paras["mim_recessive"]
        )
        print("Error: Please download it from the source website")
        sys.exit()
    ...
    try:
        ...
        print(
            "Error: can't read the OMIM dominant disorder file %s" % paras["mim_domin"]
        )
        print("Error: Please download it from the source website")
        sys.exit()
    else:
    ...
    try:
        ...
        print(
            "Error: can't read the OMIM adult onset disorder file %s"
            % paras["mim_adultonset"]
        )
        print("Error: Please download it from the source website")
        sys.exit()
    else:
...
def check_downdb():
    ...
    ds.expandtabs(1)
    # database_names = refGene 1000g2014oct esp6500siv2_all avsnp151 ljb26_all clinvar_20150629 gnomad_genome hg19_dbscsnv11 dbnsfp31a_interpro rmsk ensGene
...
def check_annovar_result():
    # table_annovar.pl example/ex1.avinput humandb/ -buildver hg19 -out myanno -remove -protocol refGene,esp6500siv2_all,1000g2015aug_all,avsnp151,ljb26_all,CLINSIG,gnomad_genome   -operation  g,f,f,f,f,f,f   -nastring . -csvout
    inputft = paras["inputfile_type"]
    annovar_options = " "
    ...
    if inputft.lower() == "avinput":
        cmd = (
            ...
            + " -remove -out "
            + paras["outfile"]
            + " -protocol refGene,esp6500siv2_all,1000g2015aug_all,avsnp151,dbnsfp47a,clinvar_20260102,gnomad_genome,dbscsnv11,rmsk,ensGene,knownGene  -operation  g,f,f,f,f,f,f,f,r,g,g   -nastring ."
            + annovar_options
        )
        print("%s" % cmd)
        os.system(cmd)
...
def classfy(PVS1, PS, PM, PP, BA1, BS, BP, Allels_flgs, cls):
    ...
    if PS_sum == 1:
        if PM_sum == 1 or PM_sum == 2:
            PAS_out = 1
    if PVS1 == 1:
        if PM_sum == 1:
            PAS_out = 1  # 1:Likely pathogenic
    if PS_sum == 1 and PP_sum >= 2:
        PAS_out = 1
    if PM_sum >= 3:
        PAS_out = 1
    if PM_sum == 2 and PP_sum >= 2:
        PAS_out = 1
    if PM_sum == 1 and PP_sum >= 4:
        PAS_out = 1

    if PVS1 == 1:
        if PS_sum >= 1:
            PAS_out = 0  # 0:Pathogenic
        if PM_sum >= 2:
            PAS_out = 0
        if PM_sum == 1 and PP_sum == 1:
            PAS_out = 0
        if PP_sum >= 2:
            PAS_out = 0
    if PS_sum >= 2:
    ...
    if BS_sum == 1 and BP_sum == 1:
        BES_out = 3  # 3:Likely benign
    if BP_sum >= 2:
        BES_out = 3
    if BA1 == 1 or BS_sum >= 2:
        BES_out = 2  # 2:Benign

    if PAS_out != -1 and BES_out == -1:
        BPS_out = PAS_out
    if PAS_out == -1 and BES_out != -1:
        BPS_out = BES_out
    if PAS_out == -1 and BES_out == -1:
    ...
    for cls0 in re.split("[,;]", line_tmp2):
        ...
        if len(cls0_1) > 1:
            ...
            try:
                ...
                try:
                    if (
                        float(knownGeneCanonical_ed_dict[trans_id])
                        - float(cls[Allels_flgs["Start"]])
                    ) < 50:  # means close  3' of gene 50 bp.
                        PVS = 0
                except ValueError:
                    pass
                else:
...
def check_PS1(line, Funcanno_flgs, Allels_flgs, aa_changes_dict):
    ...
    try:
        if (
            float(cls[Funcanno_flgs["dbscSNV_RF_SCORE"]]) > dbscSNV_cutoff
            or float(cls[Funcanno_flgs["dbscSNV_ADA_SCORE"]]) > dbscSNV_cutoff
        ):  # means alter the splicing
            PS1_t3 = 1
        if (
            cls[Funcanno_flgs["dbscSNV_RF_SCORE"]] == "."
            or cls[Funcanno_flgs["dbscSNV_ADA_SCORE"]] == "."
        ):  # absent also means not in splicing
            PS1_t3 = 0
    except ValueError:
        pass
    else:
...
def check_PM2(
    line, Freqs_flgs, Allels_flgs, Funcanno_flgs, mim2gene_dict, mim2gene_dict2
):
    """
    ...
    if (
        tt == 0
    ):  # means some controls has frequency and it is not absent; then need to check it is recessive or not
        tt2 = 1
        mim_num = 0
        try:
                ...
                if mim_recessive_dict[mim_num] == "1":  # it is recessive
                    for key in Freqs_flgs.keys():
                        ...
                        try:
                            if (
                                cls[Freqs_flgs[key]] != "."
                                and float(cls[Freqs_flgs[key]]) >= cutoff_maf
                            ):
                                tt2 = tt2 * 0
                        except ValueError:
                            pass
...
def check_PP5(line, Funcanno_flgs, Allels_flgs):
    ...
    if line_tmp2 != ".":
        ...
        clinvar_bp = cls3[0]
        if (
            clinvar_bp.find("ikely pathogenic") >= 0
            or clinvar_bp.find("athogenic") >= 0
        ):
...
def check_BA1(line, Freqs_flgs, Allels_flgs):
    ...
    for key in Freqs_3pops.keys():
        try:
            if float(cls[Freqs_flgs[key]]) > 0.05:
                BA1 = 1
    ...
    for key in Freqs_flgs.keys():
        try:
            if cls[Freqs_flgs[key]] != ".":
                if float(cls[Freqs_flgs[key]]) >= cutoff:
                    BS1 = 1
        except ValueError:
            pass
        else:
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                ):  # means recessive disorder: check snps as homo
                    try:
            ...
            try:
                if (
                    mim_domin_dict[mim1] == "1"
                ):  # means dominant disorder: check snps as heter
                    BS2 = 0
                    try:
...
def check_BP3(line, Funcanno_flgs, Allels_flgs):
    ...
    if (
        cls[Funcanno_flgs["rmsk"]] != "."
        and cls[Funcanno_flgs["Interpro_domain"]] == "."
    ):  # repeat and not in domain
        BP3_t2 = 1
...
def check_BP4(line, Funcanno_flgs, Allels_flgs):
    ...
    try:
        if (
            float(cls[Funcanno_flgs["dbscSNV_RF_SCORE"]]) <= dbscSNV_cutoff
            and float(cls[Funcanno_flgs["dbscSNV_ADA_SCORE"]]) <= dbscSNV_cutoff
        ):
            BP4_t3 = 1
    except ValueError:
    ...
    for fc in funcs_tmp:
        if line_tmp.find(fc) >= 0 and line_tmp.find(funcs_tmp2) < 0:
            ...
            if (
                ...
                if (
                    float(cls[Funcanno_flgs["dbscSNV_RF_SCORE"]]) < 0.6
                    and float(cls[Funcanno_flgs["dbscSNV_ADA_SCORE"]]) < 0.6
                ):
                    BP7_t1 = 1
    # check the conservation score of gerp++ > 2
    try:
        if (
            float(cls[Funcanno_flgs["GERP++_RS"]]) <= float(cutoff_conserv)
            or cls[Funcanno_flgs["GERP++_RS"]] == "."
        ):
            BP7_t2 = 1
    except ValueError:
        # absent means there are gaps in the multiple alignment,so cannot have the score,not conserved
...
def assign(BP, line, Freqs_flgs, Funcanno_flgs, Allels_flgs):
                ...
                if len(evd_t) > 1 and (
                    ...
                    if int(evd_t[1]) <= 1:
                        ...
                        if evd_t[0].find("PS") != -1:
                            ...
                            tt3 = int(tt[t + 2 : t + 3])
                            if t < len(evd_t[0]) - 2 and tt3 <= 5:
                                PS[tt3 - 1] = int(evd_t[1])
                        if evd_t[0].find("PM") != -1:
                            t = evd_t[0].find("PM")
                            tt = evd_t[0]
                            tt3 = int(tt[t + 2 : t + 3])
                            if t < len(evd_t[0]) - 2 and tt3 <= 7:
                                PM[tt3 - 1] = int(evd_t[1])
                        if evd_t[0].find("PP") != -1:
                            t = evd_t[0].find("PP")
                            tt = evd_t[0]
                            tt3 = int(tt[t + 2 : t + 3])
                            if t < len(evd_t[0]) - 2 and tt3 <= 6:
                                PP[tt3 - 1] = int(evd_t[1])
                        if evd_t[0].find("BS") != -1:
                            t = evd_t[0].find("BS")
                            tt = evd_t[0]
                            tt3 = int(tt[t + 2 : t + 3])
                            if t < len(evd_t[0]) - 2 and tt3 <= 5:
                                BS[tt3 - 1] = int(evd_t[1])
                        if evd_t[0].find("BP") != -1:
                            t = evd_t[0].find("BP")
                            tt = evd_t[0]
                            tt3 = int(tt[t + 2 : t + 3])
                            if t < len(evd_t[0]) - 2 and tt3 <= 8:
                                BP[tt3 - 1] = int(evd_t[1])
    ...
    if len(cls) > 1:  # esp6500siv2_all 1000g2015aug_all gnomAD_genome_ALL
        ...
        line_t = "%s PVS1=%s PS=%s PM=%s PP=%s BA1=%s BS=%s BP=%s" % (
            BP_out,
            PVS1,
            PS,
            PM,
...
def my_inter_var(annovar_outfile):
    ...
    try:
        ...
        if re.findall("true", paras["otherinfo"], flags=re.IGNORECASE):
            fw.write(
                "#%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t InterVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
                % (
                    "Chr",
                    "Start",
                    "End",
            ...
            fw.write(
                "#%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t InterVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
                % (
                    "Chr",
                    "Start",
                    "End",
        ...
        for line in strs.split("\n"):
            ...
            if line_sum == 0:
                ...
                if re.findall("true", paras["otherinfo"], flags=re.IGNORECASE):
                    cls[Funcanno_flgs["Otherinfo"]] = cls[
                        Funcanno_flgs["Otherinfo"]
                    ].replace("\t", ";")
                    fw.write(
                        "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\tclinvar: %s \t InterVar: %s \t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
                        % (
...
def main():
    ...
    parser.add_option(
        ...
        dest="buildver",
        action="store",
        help="The genomic build version, it can be hg19 or hg38 , will support other version later",
        metavar="hg19",
    )
    ...
    parser.add_option(
        ...
        dest="output",
        action="store",
        help="The prefix of output file which contains the results, the file of results will be as [$$prefix].intervar ",
    ...
    if os.path.isfile(config_file):
        ...
        print(
            "Error: The default configure file of [ config.ini ] is not here, exit! Please redownload the InterVar."
        )
        sys.exit()
    ...
    if options.evidence_file != None:
        ...
        print(
            "Warning: You provided your own evidence file [ %s ] for the InterVar."
    ...
    if options.database_intervar != None:
        ...
        paras["knowngenecanonical"] = (
            paras["database_intervar"] + "/knownGeneCanonical.txt"
        )
    ...
    if options.skip_annovar != True:
        ...
        print(
            "Warning: The InterVar will interpret the variants based on your old annotation information!"
        )

    cutoff = float(paras["disorder_cutoff"])  # user's disorder cutoff
    if cutoff != 0.005:
        print(
            "Warning: Customized  disease cutoff %s in config.ini, not suggested as 0.005"
            % (cutoff)
        )
    ...
    for annovar_outfile in glob.iglob(
        ...
        if os.path.isfile(outfile):
            ...
            print(
                "Notice: About %d variants has been processed by InterVar" % (sum2 - 1)
            )
            if inputft.lower() != "vcf_m":
                print(
                    "Notice: The InterVar is finished, the output file is [ %s.intervar ]"
                    % annovar_outfile
                )
        else:
            some_file_fail = some_file_fail + 1
            print(
                "Warning: The InterVar seems not run correctly, please check your inputs and options in configure file"
            )

    if inputft.lower() == "vcf_m":
        print(
            "Notice: The InterVar for VCF with multiple samples is finished, the output file is as [ %s.<samplename>.intervar ]"
            % annovar_outfile
        )
        sum_sample = 1
        for f in glob.iglob(
            paras["outfile"] + "*." + paras["buildver"] + "_multianno.txt.intervar"
        ):
            print(
                "Notice: The InterVar for VCF with multiple samples is finished, The %d sample output file is [ %s]"
                % (sum_sample, f)
            )
            sum_sample = sum_sample + 1
        if some_file_fail >= 1:
            print(
                "Warning: The InterVar seems not run correctly for your %d samples in the VCF, please check your inputs and options in configure file"
                % some_file_fail
            )
    if out_annf == 0:
        print(
        ...
        print(
            "ERROR: The name of annotation result file should be like %s*.%s__multianno.txt"
