...
def parseOptions():
    ...
    parser = ArgumentParser(
        description=desc, formatter_class=RawDescriptionHelpFormatter, epilog=epilog
    )
    parser.add_argument(
        "ds_file",
        type=str,
        help="COSMIC datasource filename. For example, 'CosmicFusionExport.tsv' ",
    )
    parser.add_argument(
        "output_file",
        type=str,
        help="TSV filename for output.  File will be overwritten if it already exists.",
    )
...
def renderFusionGeneDictEntry(geneKey, fusionGeneDict):
    ...
    for k in fusionGeneSubDict.keys():
        ...
        summaryString = "%(k)s(%(fgene)s)" % {
            "k": k,
            "fgene": str(fusionGeneSubDict[k]),
        }
...
if __name__ == "__main__":
    ...
    if "TRANSLOCATION_NAME" not in headers:
        raise NotImplementedError(
            "Could not find TRANSLOCATION_NAME column in the input file."
        )
    ...
    for i, line in enumerate(tsvReader):
        ...
        genes_in_this_fusion = re.findall(
            "\(([A-Z0-9\-\.]+)\)", fusion_gene_description
        )

        print(genes_in_this_fusion)
        ...
        for k in genes_in_this_fusion:
            ...
            fusionGeneDict[k][fusion_gene_description] = (
                fusionGeneDict[k][fusion_gene_description] + 1
            )
    ...
    tsvWriter = csv.DictWriter(
        open(outputFilename, "w"), outputHeaders, delimiter="\t", lineterminator="\n"
    )
