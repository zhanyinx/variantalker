...
function usage() {
    ...
    echo "Usage: updateCosmicDataBase.sh passcode version"
    echo "Passcode is generated by "
    echo "echo 'email@example.com:mycosmicpassword' | base64"
    echo "source:"
    echo "    cosmic"
    echo "    cosmic_tissue"
    echo "    cosmic_fusion"
...
mkdir -vp cosmic/hg38 cosmic_fusion/hg38 cosmic_tissue/hg38
mkdir -vp cosmic/hg19 cosmic_fusion/hg19 cosmic_tissue/hg19
...
link=$(curl -H "Authorization: Basic $passcode" https://cancer.sanger.ac.uk/cosmic/file_download/GRCh37/cosmic/$version/CosmicCompleteTargetedScreensMutantExport.tsv.gz)
url=`echo ${link::-2} | cut -d"\"" -f4-`
curl ${url} --output cosmic/hg19/CosmicCompleteTargetedScreensMutantExport.tsv.gz

link=$(curl -H "Authorization: Basic $passcode" https://cancer.sanger.ac.uk/cosmic/file_download/GRCh37/cosmic/$version/CosmicFusionExport.tsv.gz)
url=`echo ${link::-2} | cut -d"\"" -f4-`
curl ${url} --output cosmic_fusion/hg19/CosmicFusionExport.tsv.gz

link=$(curl -H "Authorization: Basic $passcode" https://cancer.sanger.ac.uk/cosmic/file_download/GRCh38/cosmic/$version/CosmicCompleteTargetedScreensMutantExport.tsv.gz)
url=`echo ${link::-2} | cut -d"\"" -f4-`
curl ${url} --output cosmic/hg38/CosmicCompleteTargetedScreensMutantExport.tsv.gz

link=$(curl -H "Authorization: Basic $passcode" https://cancer.sanger.ac.uk/cosmic/file_download/GRCh38/cosmic/$version/CosmicFusionExport.tsv.gz)
url=`echo ${link::-2} | cut -d"\"" -f4-`
curl ${url} --output cosmic_fusion/hg38/CosmicFusionExport.tsv.gz
