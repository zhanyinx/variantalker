process run_funcotator_single{
    ...
    if [ "${meta.sample_type}" == "germline" ]; then
        ...
        normal="\$(cat ${vcf} | grep 'normal_sample'  | cut -d'=' -f2)"
        tumor="\$(cat ${vcf} | grep 'tumor_sample'  | cut -d'=' -f2)"
        extra_params=" --annotation-default Matched_Norm_Sample_Barcode:\${normal} --annotation-default Tumor_Sample_Barcode:\${tumor} --annotation-default Tumor_type:${meta.tumor_tissue} --data-sources-path ${safe_funcotator_somatic_db}"
    fi
